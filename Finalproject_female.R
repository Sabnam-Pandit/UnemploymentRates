...
unemployment_rate <- ts(data$unrate_women, start = c(1948, 1), end = c(2015, 12), frequency = 12)
unemployment_rate

all_unemployment_rate = ts(data$unrate_women, start = c(1948, 1), frequency = 12)
length(all_unemployment_rate)


# Plot the time series data
plot(unemployment_rate, main = "US Unemployment Rate Women (1948-2015)", ylab = "Rate", xlab = "Year")
...
#ARIMA(1,0,2)xSARIMA(0,0,1)
out2 = arima(logx, order = c(1, 1, 2), seasonal = list(order=c(0,1,1),period = 12))
out2
coeftest(out2)
out2$aic
...
#ARIMA(1,0,6) x SARIMA(0,0,1)

out22 = arima(logx, order = c(1, 1, 6), seasonal = list(order=c(0,1,1),period = 12))
out22
coeftest(out22)
out22$aic

out22.1 = arima(logx, order = c(1,1,6), seasonal = list(order=c(0,1,1), period=12), fixed=c(NA,NA,0,NA,NA,NA,NA,NA))
out22.1
coeftest(out22.1)
out22.1$aic

Box.test(out22.1$residuals, type = 'Ljung',lag = 60)
par(mfrow=c(1,2))
...
polyroot(c(1, -out22.1$coef[1])) 
abs(polyroot(c(1, out22.1$coef[2:7])))
...
all_unemployment_rate = ts(data$unrate_women, start = c(1948, 1), end = c(2020, 12),frequency = 12)
...
plot(all_unemployment_rate, xlim = c(2008, 2021), ylim = c(min(all_unemployment_rate), max(all_unemployment_rate)), type = 'o', xlab = "Year", ylab = "Rate", main = "US Unemployment Rate Prediction for Women")
lines(pred, col = 'red')
